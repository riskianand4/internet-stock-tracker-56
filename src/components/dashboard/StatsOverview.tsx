...
const StatsOverview = () => {
  ...
  const lowStockCount = (stats as any)?.lowStockCount || products?.filter(p => p.status === 'low_stock').length || 0;
  const outOfStockCount = (stats as any)?.outOfStockCount || products?.filter(p => p.status === 'out_of_stock').length || 0;
