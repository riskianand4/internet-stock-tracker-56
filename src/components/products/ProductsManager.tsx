...
const ProductsManager = () => {
  const logger = createComponentLogger('ProductsManager');
  const {
    user
  } = useApp();
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (categoryFilter !== 'all') {
      ...
      logger.debug('After category filter', {
        count: filtered.length
      });
    }
    ...
    if (statusFilter !== 'all') {
      filtered = filtered.filter(product => {
        const matches = product.status === statusFilter;
        if (!matches) {
          logger.debug('Product status check', {
    ...
    logger.debug('Final filtered products', {
      count: filtered.length
    });
    return filtered;
  ...
  const handleBulkAction = (action: string) => {
    ...
    switch (action) {
      case 'delete':
        ...
        logger.info(`Deleting ${selectedProducts.length} products`, {
      ...
      case 'category':
        ...
        logger.info(`Changing category for ${selectedProducts.length} products`);
        break;
    }
  };
  ...
  return <div className="min-h-screen bg-muted/10 mobile-responsive-padding py-3 md:py-6">
      ...
      y: 0
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 md:gap-4">
          <div>
            <h1 className="mobile-text-small text-primary font-bold md:text-lg">
              Manajemen Produk
            </h1>
            <p className="text-muted-foreground mobile-text-tiny md:text-base mt-1">
              Kelola inventory dan produk ISP Anda
            </p>
          </div>
          
          <div className="flex flex-wrap items-center mobile-gap-tight">
            {isAdmin && <>
                <Button variant="outline" onClick={handleImport} size="sm" className="flex-1 md:flex-none h-7 md:h-9">
                  <Upload className="w-3 h-3 md:w-4 md:h-4 md:mr-2" />
                  <span className="hidden md:inline ml-2">Import</span>
                </Button>
                <Button variant="outline" onClick={handleExport} size="sm" className="flex-1 md:flex-none h-7 md:h-9">
                  <Download className="w-3 h-3 md:w-4 md:h-4 md:mr-2" />
                  <span className="hidden md:inline ml-2">Export</span>
                </Button>
                <Button onClick={() => setIsAddDialogOpen(true)} size="sm" className="flex-1 md:flex-none h-7 md:h-9">
                  <Plus className="w-3 h-3 md:w-4 md:h-4 md:mr-2" />
                  <span className="hidden md:inline ml-2">Tambah</span>
                </Button>
              </>}
            {!isAdmin && <Button variant="outline" onClick={handleExport} size="sm" className="flex-1 md:flex-none h-7 md:h-9">
                <Download className="w-3 h-3 md:w-4 md:h-4 md:mr-2" />
                <span className="hidden md:inline ml-2">Export</span>
              </Button>}
          </div>
        ...
        <Card className="glass">
          <CardContent className="mobile-padding-compact md:p-6">
            <div className="mobile-spacing-tight md:space-y-4">
              {/* Search */}
              <div className="relative w-full">
                ...
                <Input placeholder="Cari produk, SKU, atau kategori..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-8 md:pl-10 h-8 md:h-10 mobile-text-small" />
              </div>

              {/* Filters */}
              <div className="grid grid-cols-2 md:grid-cols-4 lg:flex lg:flex-wrap mobile-gap-tight">
